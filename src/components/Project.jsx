...
const Project = ({img,shade,icon,clas}) => {
  return (
      <div className='project-box'>
        ...
        <div ref={icon} className='icons' onMouseEnter={(e) =>{shade.current.style.display='none';icon.current.style.display='flex'}} onMouseLeave={(e) =>{shade.current.style.display='block';icon.current.style.display='none'}}>
          <a href='https://github.com/mvaibhavprakash/' target='__blank' className={`iconn ${clas}`} ><FontAwesomeIcon icon={faUpRightFromSquare}/></a>
          <a href='https://github.com/mvaibhavprakash/' target='__blank' className={`iconn ${clas}`} ><FontAwesomeIcon icon={faGithub}/></a>
        </div>
